...
parser.add_argument('--hidden_sizes_actor', type=list, default=[64, 128, 128, 128, 64])
parser.add_argument('--hidden_sizes_critic', type=list, default=[128, 256, 256, 64])
parser.add_argument('--hidden_sizes_var', type=list, default=[128, 256, 256, 64])
...
def cnn_mlp(x, hidden_sizes=(64,), activation=tf.tanh, output_activation=None, cnn_sizes=[16, 32]):
    ...
    if cnn_sizes:
        ...
        for i in cnn_sizes:
            x = tf.layers.conv1d(x, filters=i, kernel_size=3, padding='same',
                                 use_bias=False, activation=None, name='cnn_%d' % j)
            x = tf.layers.batch_normalization(x, training=True, name='bn_%d' % j)
            x = tf.tanh(x)
            j += 1
...
def get_target_update(main_name, target_name, polyak):
    ...
    for v_targ in targ_vars:
        ...
        assign_ops.append(tf.assign(targ_vars[v_targ],
...
def Fig(logger_Dict, item='EpCost', c=None, Title=None, SaveFig=True, tsplot=True):
    try:
        ...
        if tsplot:
            Epoch = np.concatenate((Epoch, Epoch, Epoch))
    ...
    def __init__(self, obs_dim, act_dim, size, load_buffer=True, Path_xls='./logger/Buffer.xlsx'):
        ...
        if load_buffer:
            try:
                ...
                self.costs_buf[:u] = np.array(excel_data['costs'])[:u].reshape(-1)
...
def RSAC(env_fn, actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var,
    ...
    with tf.variable_scope('main'):
        ...
        qc, qc_pi = critic_fn(x_ph, a_ph, pi, name='qc', **ac_kwargs_critic)
        qc_var, qc_pi_var = var_fn(x_ph, a_ph, pi, name='qc_var', **ac_kwargs_var)
    ...
    with tf.variable_scope('target'):
        ...
        _, qc_pi_targ = critic_fn(x2_ph, a_ph, pi2, name='qc', **ac_kwargs_critic)
        _, qc_pi_var_targ = var_fn(x2_ph, a_ph, pi2, name='qc_var', **ac_kwargs_var)
    ...
    if proc_id() == 0:
        ...
        print('\nParams: pi:%d, qr1:%d, qr2:%d, qc:%d, qc_v:%d, total:%d\n' % var_counts)
    ...
    damp = damp_scale * tf.reduce_mean(cost_constraint_val - qc - pdf_cdf * tf.sqrt(qc_var))

    # ---- 损失函数 ----
    pi_loss = tf.reduce_mean(alpha * logp_pi - min_q_pi
                             + (beta - damp) * (qc_pi + pdf_cdf * (qc_pi_var ** 0.5)))
    ...
    if use_costs:
        cost_constraint = cost_constraint if cost_constraint is None else cost_constraint_val
    ...
    if fixed_entropy_bonus is None:
        with tf.control_dependencies([train_q_op]):
            train_entreg_op = MpiAdamOptimizer(learning_rate=lr_ph).minimize(alpha_loss, var_list=get_vars('entreg'))

    if use_costs and fixed_cost_penalty is None:
        deps = [train_entreg_op] if fixed_entropy_bonus is None else [train_q_op]
        with tf.control_dependencies(deps):
            train_costpen_op = MpiAdamOptimizer(learning_rate=lr_ph * lr_scale).minimize(beta_loss, var_list=get_vars('costpen'))
    ...
    def test_agent(num_wind=0, deterministic=True, n_test_sce=100):
        ...
        ep_rU, ep_rH, ep_rP, ep_cV, ep_cD, ep_cH = 0, 0, 0, 0, 0, 0
        u_g = np.zeros((n_gen, n_T))
        ROCOF_t, f_Nadir_t, f_Qss_t, curtailment_t, shedding_t = (np.zeros(n_T) for _ in range(5))

        for i in range(max_ep_len):
            o2, r, c, d, reward_u, reward_HESS, reward_p, cost_vio, cost_deta, cost_hess, \
                ROCOF, f_Nadir, f_Qss, curtailment, shedding, u_state = \
                test_env.step(get_action(o, deterministic), o, ep_len, Train=Train, n_test_sce=n_test_sce)

            ep_ret += r; ep_cost += c; ep_len += 1
            ep_rU += reward_u; ep_rH += reward_HESS; ep_rP += reward_p
            ep_cV += cost_vio; ep_cD += cost_deta; ep_cH += cost_hess
        ...
        return (ep_ret, ep_cost, ep_len, ep_rU, ep_rH, ep_rP,
                ep_cV, ep_cD, ep_cH, u_g, ROCOF_t, f_Nadir_t, f_Qss_t, curtailment_t, shedding_t)
    ...
    if not Train:
        ...
        ep_ret, ep_cost, ep_len, ep_rU, ep_rH, ep_rP, ep_cV, ep_cD, ep_cH, \
            u_g, ROCOF_t, f_Nadir_t, f_Qss_t, curtailment_t, shedding_t = \
            test_agent(num_wind=num_wind, deterministic=True, n_test_sce=n_test_sce)

        print("求解时间:%.4f秒" % (time.time() - start))
        print('\n==================Test Result===================')
        print(f'     The value of Ep_ret      |   {ep_ret:.2f}')
        print(f'     The value of Ep_cost     |   {ep_cost:.2f}')
        print('----------------------             ---------------')
    ...
    # ==================== 训练阶段 ====================
        ...
        vars_to_get = dict(LossPi=pi_loss, LossQR1=qr1_loss, LossQR2=qr2_loss,
                           LossQC=qc_loss, LossQCVar=qc_var_loss,
                           QR1Vals=qr1, QR2Vals=qr2, QCVals=qc, QCVar=qc_var,
        ...
        cum_cost, local_steps = 0, 0
        local_steps_per_epoch = steps_per_epoch // num_procs()
        local_batch_size = batch_size // num_procs()
        epoch_start_time = time.time()
        steps_per_epoch_total = local_steps_per_epoch * max_ep_len

        for t in range(total_steps // num_procs()):
            # 动态学习率: 前1000代 1e-3, 1000~8000代 1e-4
            current_epoch = t // steps_per_epoch_total
            current_lr = 1e-3 if current_epoch < 1000 else 1e-4

            a = get_action(o) if t > local_start_steps else env.action_sample()

            o2, r, c, d = env.step(a, o, ep_len)
            r *= reward_scale
            ep_ret += r; ep_cost += c; ep_len += 1; local_steps += 1
            cum_cost += c
            ...
            if d or (ep_len == max_ep_len):
                ...
                o, r, d, ep_ret, ep_cost, ep_len = \
                    env.reset(n=np.random.randint(n_wind_data)), 0, False, 0, 0, 0
            ...
            if t > 0 and t % update_freq == 0:
                for j in range(update_freq):
                    ...
                    feed_dict = {x_ph: batch['obs1'], x2_ph: batch['obs2'], a_ph: batch['acts'],
                                 r_ph: batch['rews'], c_ph: batch['costs'],
                                 d_ph: batch['done'], lr_ph: current_lr}

                    if t < local_update_after:
                        values = sess.run(vars_to_get, feed_dict)
                    else:
                        values = sess.run([vars_to_get, grouped_update], feed_dict)[0]
                    logger.store(**values)

                ETA = int((time.time() - epoch_start_time) * (total_steps // update_freq - t // update_freq))
                print(f'Training: [epoch:{current_epoch}|{epochs - 1}] '
                      f'[batch:{t % steps_per_epoch_total // max_ep_len}|{steps_per_epoch - 1}] '
                      f'ETA {ETA // 3600}:{ETA % 3600 // 60}:{ETA % 60} | LR: {current_lr}')
                epoch_start_time = time.time()

            if t > 0 and t % steps_per_epoch_total == 0:
                epoch = t // steps_per_epoch_total
                cost_rate = mpi_sum(cum_cost) / ((epoch + 1) * steps_per_epoch)
                ...
                logger.log_tabular('LossQC', with_min_and_max=True)
                logger.log_tabular('LossQCVar', with_min_and_max=True)
...
if __name__ == '__main__':
    ...
    RSAC(lambda: unPower(uncertain=True, n_sce=5), actor_fn=mlp_actor, critic_fn=mlp_critic, var_fn=mlp_var,
    ...
    if args.Train:
        ...
        Fig(logger_Dict, 'EpCost', c="#FA7F6F", SaveFig=args.SaveFig, tsplot=True)
        Fig(logger_Dict, 'CostRate', SaveFig=args.SaveFig, tsplot=False)
        ...
        Fig(logger_Dict, 'LossQC', c="#E6B33D", SaveFig=args.SaveFig, tsplot=True)
        Fig(logger_Dict, 'LossQR1', c="#E6B33D", SaveFig=args.SaveFig, tsplot=True)
        Fig(logger_Dict, 'LossQCVar', c="#E6B33D", SaveFig=args.SaveFig, tsplot=True)
